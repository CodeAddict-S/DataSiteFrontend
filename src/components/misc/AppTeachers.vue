...
const save = () => {
    api.patch(`teachers/${rowToEdit.value.id}/`, rowToEdit.value).then(() => {
...
const newRow = ref({})
const create = () => {
    api.post('teachers/', newRow.value).then(() => {
        showToast('success', t('success'))
        fetchRelated()
    })
}
...
const filteredObjects = computed(() => {
    return objects.value.filter(s => s?.fullname.toLowerCase()?.match(search.value.toLowerCase()))
})

const fetchRelated = () => {
    api('teachers/').then(res => {
